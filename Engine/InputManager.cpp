...
float InputManager::GetAxisAbsolute(int id, eDevice device, int index)
{
	...
	case eDevice::CONTROLLER:
		assert(index < static_cast<int>(m_controllers.size()));
...
InputManager::eButtonState InputManager::GetButtonState(int id, eDevice device, int index)
{
	...
	bool prevButtonDown = GetPrevButtonDown(id, device, index);
	if (buttonDown)
	{
...
bool InputManager::GetPrevButtonDown(int id, eDevice device, int index)
{
	bool buttonDown = false;
